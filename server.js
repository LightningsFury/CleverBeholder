...
client.on('ready', async () => {
  ...
  let time;
    const m = await client.channels.get('506167081805283331')
        .send({
            embed: {
                color: 4447003,
...
client.on('message', async msg=> {
  ...
  let remind = async (max) => {
    ...
    sent.delete(5000)
    msg.guild.channels.get('506167081805283331').send('<@!326483019349098506> '+msg.content+' '+max)
  }
  ...
  if (msg.channel.id !== "506166393893421073") return;
  ...
  let max = ((recentMsgs.length / 10) * 10) * 0.7;
  max = max < 1 ? 3 : max;
